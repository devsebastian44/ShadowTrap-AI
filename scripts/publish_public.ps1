...
# Proyecto: ShadowTrap-AI (Honeypot)
# =============================================================================

Write-Host "[*] Iniciando sincronización profesional de ShadowTrap-AI Honeypot..." -ForegroundColor Cyan
...
Remove-Item -Path "__pycache__", ".pytest_cache", ".venv", "venv", "env" -Recurse -Force -ErrorAction SilentlyContinue
...
git rm -r --cached cowrie-config/ -f 2>$null
git rm -r --cached backend/tests/ -f 2>$null
